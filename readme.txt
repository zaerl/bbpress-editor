=== zaerl Editor ===
Contributors: zaerl
Tags: quicktags, texture, editor, zaerl
Requires at least: 1.0.2
Tested up to: 1.0.2
Stable tag: 0.3.2

A better Editor for bbPress. This plugin is no longer actively developed and is saved for historical reasons.

== Description ==

zaerl Editor adds a little toolbar with useful buttons on top of the editor area. Each of these buttons automatically adds the html code of one of the allowed markup. No template edit required. You can specify a custom CSS for the toolbar as well as the CSS selector used to find the textarea.

jQuery is used in order to generate the HTML code needed and the code is loaded only on those template page that actually have a post form. Italian and spanish translations are included.

== Installation ==

Extract the zip file and just drop the contents in the my-plugins/ directory of your bbPress installation and then activate the Plugin from Plugins page.

== Screenshots ==

1. The toolbar

== Changelog ==

= 0.3.2 =
* Support for "Create New topic" form

= 0.3.1 =
* Fixed the CSS selector loading
